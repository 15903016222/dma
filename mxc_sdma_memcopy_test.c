								...
								loff_t * offset)
{
	...
	for (i = 0; i < 10; ++i) {
		printk ("*(rbuf2 + %d) = 0x%x -- *(wbuf2 + %d) = 0x%x \n", i, *(rbuf2 + i), i, *(wbuf2 + i));
	}
	...
	for (i = 0; i < 10; ++i) {
		printk ("*(rbuf3 + %d) = 0x%x -- *(wbuf3 + %d) = 0x%x \n", i, *(rbuf3 + i), i, *(wbuf3 + i));
	}
								...
								loff_t * offset)
{
	...
	dma_m2m_desc = dma_m2m_chan->device->device_prep_slave_sg(dma_m2m_chan,sg, 3, dma_m2m_config.direction, 1, NULL);
	...
	dma_m2m_desc = dma_m2m_chan->device->device_prep_slave_sg(dma_m2m_chan,sg2, 3, dma_m2m_config.direction, 0, NULL);
